...
import {
  ...
  sizeValues,
  type IconSize,
} from "@inkabledesign/mariner-theme";
...
export type IconProps<T extends IconCategoryType = IconCategoryType> = {
  ...
  iconSize?: IconSize; // Semantic size token (xxxs through xxxl)
 ...
 *
 * @example
 * // Semantic sizing from the shared size tokens
 * <Icon iconName="ico-heart" iconSize="lg" />
 *
 * @example
 ...
 * />
 */
const Icon = <T extends IconCategoryType = IconCategoryType>(
  props: IconProps<T>,
) => {
  const {
  ...
  const iconType =
    explicitIconType && iconName in iconMap[explicitIconType]
      ? explicitIconType
      : findIconType(iconName);
  ...
  const sizeClasses = iconSize || hasCustomSize ? "" : defaultSize;
  const tokenSize = iconSize ? sizeValues[iconSize] : undefined;
  const tokenSizeStyle = tokenSize
  ...
  return (
    <ViewStyled
      ...
      style={tokenSizeStyle ? [tokenSizeStyle, style] : style}
    >
      <IconComponent
        width="100%"
        height="100%"
